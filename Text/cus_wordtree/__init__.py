...
def build_both_trees(keyword, ngrams, frequencies):
    ...
    for ngram, freq in zip(ngrams, frequencies):
        fwd = front_match(keyword, ngram)
        bwd = front_match(keyword[::-1], ngram[::-1])
        assert fwd or bwd, "ngram does not have keyword at beginning or end: {}".format(ngram)

        if fwd:
            fwd_ngrams.append(ngram[N:])
            fwd_frequencies.append(freq)
        if bwd:
            bwd_ngrams.append(reversed(ngram[:-N]))
...
class TreeDrawer:
    def __init__(self, keyword, fwd_tree, bwd_tree, max_font_size=30, min_font_size=12, font_interp=None):
        ...
        self.keyword = to_string(keyword)
        ...
        self.graph = Digraph(self.keyword, format='png')
...
def search(corpus, keyword, max_n=5, tokenizer=None):
    ...
    for doc in tqdm(corpus):
        ...
        for n in range(len(keyword)+1, max_n + len(keyword)):
            for i in range(0, len(tokens) - n + 1):
                ngram = tokens[i:i + n]
                if front_match(keyword, ngram) or front_match(keyword[::-1], ngram[::-1]):
