...
slider = Slider(start=1800, end=2021, value=1800, step=1, title="year", callback_policy = 'mouseup')
mytext = Label(x=820, y=510, x_units='screen', y_units='screen',
                 text=str(slider.value), render_mode='css',
                 border_line_color='black', border_line_alpha=1.0,
                 background_fill_color='white', background_fill_alpha=1.0)
callback = CustomJS(args=dict(source=country_source, slider=slider, mytext=mytext), code="""
    data = source.data;
    data['SELECT_YEAR'] = data[String(slider.value)]
    mytext.text = String(slider.value)
    source.change.emit();
""")
...
p.add_tools(HoverTool(renderers = [country],
                      tooltips = [('State','@STATE_NAME'),("(x,y)", "($x, $y)"),('Country','@NAME'),('Burned Area','@SELECT_YEAR')]))
